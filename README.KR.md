...
**🪧 개인 포크 공지**: 이 프로젝트는 [Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber)를 개인적으로 사용하기 위해 포크한 것입니다. 여기에는 저에게 유용한 추가 기능과 수정 사항이 포함되어 있습니다. 저는 이 포크를 적극적으로 유지 관리하거나 업스트림과 동기화하지 않습니다. 모든 새로운 코드와 대부분의 수정 사항은 AI를 통해 생성되었습니다.
...
- **📚 ElevenLabs 발음 사전 지원** – ElevenLabs TTS가 기존 발음 사전을 ID로 불러와 재사용하여 사용자 지정 발음을 적용할 수 있습니다
- **🤖 Grok API 지원** – 새로운 무상태 LLM 제공자로 xAI Grok API 지원을 추가했습니다.
- **🎧 Whisper API 지원** – 원격 텍스트 변환 서비스를 위한 Whisper API ASR 백엔드 지원을 추가했습니다.
