...
> *(このセクションはDeepLを使用して翻訳しました。誤りがあればご容赦ください。)*
**🪧 個人フォークに関するお知らせ**：これは、私自身の使用のためにメンテナンスされている[Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber)の個人フォークです。私個人にとって有益な追加機能や修正が含まれています。私はこのフォークを積極的にメンテナンスしておらず、アップストリームとの同期も行っておりません。すべての新しいコードと、ほとんどの修正はAIによって生成されました。

</h3>

## 🌟 新機能
- **📖 設定資料集のサポート** – キャラクターに設定資料集（設定項目が記載されたYAMLファイル）を紐付けることができるようになりました。AIは能動的に発言する際、この資料集を参照することで、より豊かで文脈に沿ったキャラクターの会話が可能になります
- **🎲 ランダムな能動的プロンプト** – AIは、ロアベースのプロンプト（ロアブックのエントリから）または通常の能動的プロンプトのいずれかを使用して、確率的に能動的な発話を生成するようになりました。堅牢性を確保するため、フォールバック動作も備えています
- **🗣️ TTSタスク管理の改善** – 保留中のTTSタスクの処理とペイロードキューの調整を改善するため、TTSTaskManager.wait_for_completionメソッドを一元化しました
- **🚫 エラーロギングの強化** – デバッグ診断を改善するため、TTS関連のエラーについて完全な例外トレースバックが記録されるようになりました
- **📚 ElevenLabs発音辞書のサポート** – ElevenLabs TTSで既存の発音辞書をIDで読み込み、再利用してカスタム発音を適用できるようになりました
- **🤖 Grok API のサポート** – 新しいステートレス LLM プロバイダーとして、xAI Grok API のサポートを追加しました
- **🎧 Whisper API のサポート** – リモート文字起こしサービス向けに、Whisper API の ASR バックエンドサポートを追加しました
