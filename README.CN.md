...
- **📚 ElevenLabs 发音词典支持** – ElevenLabs TTS 现在可以通过 ID 解析并复用已有的发音词典，以实现自定义发音
- **🤖 支持 Grok API** – 新增对 xAI Grok API 的支持，将其作为新的无状态大语言模型（LLM）提供商
- **🎧 支持 Whisper API** – 新增对 Whisper API 语音识别（ASR）后端的支持，用于远程转录服务
